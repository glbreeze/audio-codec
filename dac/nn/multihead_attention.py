...
from dac.nn.custom_layers import FairseqDropout
from dac.nn.quant_noise import quant_noise
from dac.nn.fairseq_incremental_decoder import FairseqIncrementalDecoder
...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def _add_bias(self, k: Tensor, v: Tensor,
        key_padding_mask: Optional[Tensor], attn_mask: Optional[Tensor],
        bsz: int, ) -> Tuple[Tensor, Tensor, Optional[Tensor], Optional[Tensor]]:
    ...
    def forward(
        ...
        Args:
            key_padding_mask (ByteTensor, optional): mask to exclude keys that are pads, of shape `(batch, src_len)`.
            need_weights (bool, optional): return the attention weights, averaged over heads (default: False).
            attn_mask (ByteTensor, optional): typically used to implement causal attention (default: None).
            before_softmax (bool, optional): return the raw attention weights and values before the attention softmax.
            need_head_weights (bool, optional): return the attention weights for each head. Implies *need_weights*. Default:
        ...
        if (
            ...
            and not static_kv
            # A workaround for quantization to work. Otherwise JIT compilation treats bias in linear module as method.
            and not torch.jit.is_scripting()
            # The Multihead attention in pytorch forces strong dimension check for input embedding dimention and K,Q,V projection dimension.
            # Pruning break the dimension check and not easy to modify the pytorch API, it is preferred to bypass the pytorch MHA when we need to skip embed_dim_check
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadAttention._append_prev_key_padding_mask(
                ...
                batch_size=kv_bsz, src_len=k.size(1), static_kv=static_kv,
            )
        ...
        if need_weights:
            ...
            if not need_head_weights: # average attention weights over heads
